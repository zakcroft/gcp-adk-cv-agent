...
diff_hash=$( { git diff HEAD -- cv_agents main.py eval tests pyproject.toml 2>/dev/null; \
               git ls-files --others --exclude-standard -- cv_agents main.py eval tests 2>/dev/null; } | shasum | cut -d' ' -f1)
...
cat <<'EOF'
{"decision": "block", "reason": "Spec check: this working tree has changes in cv_agents/, main.py, eval/ or tests/. Review whether docs/SPEC.md needs updating to stay accurate (agent tree, state keys, eval cases, config, known gaps). If the change is meaningful, update SPEC.md now; if not, briefly say so and finish. You will not be reminded again for this exact change-set."}
EOF
exit 0
